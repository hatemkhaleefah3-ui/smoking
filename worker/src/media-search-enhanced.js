...
const MAX_ENHANCED_IMAGES = 15;
const MAX_FALLBACK_QUERIES = 5;
const DESCRIPTIVE_QUERY_LENGTH = 72;
const DESCRIPTIVE_TOKEN_COUNT = 8;
...
export async function handleEnhancedMediaSearch(request, env) {
  ...
  if (!shouldExpandQuery(query, fallbackQueries) || images.length >= MAX_ENHANCED_IMAGES) {
    return baseResponse;
  }

  const fallbackEnv = {
    GEMINI_API_KEY: '',
    GEMINI_MODEL: typeof env?.GEMINI_MODEL === 'string' ? env.GEMINI_MODEL : undefined
  };
  ...
  for (const fallbackQuery of fallbackQueries) {
    ...
    try {
      fallbackResponse = await handleBaseMediaSearch(fallbackRequest, fallbackEnv);
    } catch {
      continue;
    }
    if (fallbackResponse.status !== 200) continue;

    const fallbackPayload = await fallbackResponse.json().catch(() => null);
    if (!fallbackPayload || !Array.isArray(fallbackPayload.images)) continue;
    appendImages(images, seenImages, fallbackPayload.images);
  }
...
function shouldExpandQuery(query, fallbackQueries) {
  if (!fallbackQueries.length) return false;
  const tokens = tokenize(query);
  return query.length >= DESCRIPTIVE_QUERY_LENGTH || tokens.length >= DESCRIPTIVE_TOKEN_COUNT;
}
...
function buildFallbackQueries(query) {
  ...
  const add = (value, score) => {
    ...
    if (term.split(/\s+/).length < 2) return;
  ...
  for (let size = 2; size <= 4; size += 1) {
    for (let index = 0; index + size <= tokens.length; index += 1) {
      ...
      add(window, scoreWindow(window) + (size === 3 ? 3 : size === 2 ? 2 : 0));
    }
  }
