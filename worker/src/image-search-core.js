...
const TOPIC_RULES = [
  ...
  { id: 'medical-neurological', label: 'Medical / neurological', querySuffix: 'medical neurological neurotransmitter', pattern: /neurotransmitter|neurolog|brain|nervous system|clinical|medicine|pharmacolog/i },
  ...
  { id: 'botany', label: 'Plant / botany', querySuffix: 'plant botany species photograph', pattern: /plant|botany|genus|species|flora|legume|taxonomy/i }
];
...
async function searchImages(request, env, url) {
  ...
  return json({
    ...
    feedbackRanking: {
      method: 'net-score',
      negativeRemovalThreshold: NEGATIVE_FEEDBACK_THRESHOLD
    },
    ...(debug ? { debugDiagnostics: diagnostics } : {})
  });
}
...
async function searchAllSources(query, diagnostics) {
  ...
  settled.forEach((outcome, index) => {
    ...
    const request = latestDiagnostic(diagnostics.providers, source);
    if (outcome.status === 'fulfilled') {
      sourceStatus.push({
        source,
        ok: true,
        count: outcome.value.length,
        requestUrl: request?.requestUrl || null,
        status: request?.status ?? null,
        responseType: request?.contentType || null
      });
...
async function searchWikimedia(query, diagnostics) {
  ...
  const payload = await fetchJson(endpoint, { source: 'wikimedia', stage: 'image-search', diagnostics: diagnostics.providers });
...
async function searchOpenverse(query, diagnostics) {
  ...
  const payload = await fetchJson(endpoint, { source: 'openverse', stage: 'image-search', diagnostics: diagnostics.providers });
  const results = (payload?.results || []).flatMap((item) => {
    const primaryUrl = normalizeUrl(item.url);
...
async function searchOpenI(query, diagnostics) {
  ...
  const payload = await fetchJson(endpoint, { source: 'nlm-open-i', stage: 'image-search', diagnostics: diagnostics.providers });
  ...
  const results = items.flatMap((item, index) => {
    ...
    return [{
      ...
      sourceUrl: normalizeUrl(item.detailUrl || item.sourceUrl) || `https://openi.nlm.nih.gov/detailedresult?img=${encodeURIComponent(id)}&req=4`,
...
async function detectWikidataTopics(query, diagnostics) {
  try {
    ...
    const searchPayload = await fetchJson(searchEndpoint, { source: 'wikidata', stage: 'wbsearchentities', diagnostics: diagnostics.wikidata });
    ...
    const entitiesPayload = await getWikidataEntities(entityIds, 'claims|labels|descriptions', diagnostics, 'wbgetentities-entities');
    ...
    const instancesPayload = instanceIds.size
      ? await getWikidataEntities([...instanceIds], 'labels|descriptions', diagnostics, 'wbgetentities-instances')
      : { entities: {} };
...
async function getWikidataEntities(ids, props, diagnostics, stage) {
  ...
  endpoint.searchParams.set('format', 'json');
  endpoint.searchParams.set('origin', '*');
  return fetchJson(endpoint, { source: 'wikidata', stage, diagnostics: diagnostics.wikidata });
}

export function rankResults(results, feedback, excludedUrls, topic) {
  return results.flatMap((result, providerIndex) => {
    ...
    if (feedbackScore <= NEGATIVE_FEEDBACK_THRESHOLD) return [];
    return [{
      ...result,
      imageUrl,
...
async function fetchJson(input, { source, stage, diagnostics }, timeoutMs = 10_000) {
  ...
  try {
    response = await fetch(input, {
      headers: {
        ...
        'Api-User-Agent': 'LectureStudioImageSearch/1.1 (educational image discovery)'
      },
    ...
    if (!response.ok) {
      ...
      throw new ExternalFetchError(`${source} returned HTTP ${response.status}`, entry);
    }

    try {
      return JSON.parse(rawBody);
    } catch (error) {
      logFetchFailure({ ...entry, rawResponseBody: rawBody, reason: 'invalid-json', parseError: error.message });
      throw new ExternalFetchError(`${source} returned a non-JSON response`, entry);
    }
  } catch (error) {
    ...
    throw new ExternalFetchError(`${source} request failed: ${entry.networkError}`, entry);
  } finally {
...
function limitBody(value) {
  ...
  return text.length <= DEBUG_BODY_LIMIT ? text : `${text.slice(0, DEBUG_BODY_LIMIT)}\n...[truncated ${text.length - DEBUG_BODY_LIMIT} characters]`;
}
