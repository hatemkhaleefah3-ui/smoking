...
(() => {
  ...
  function addButton(id, slotIndex) {
    ...
    input.addEventListener('change', () => {
      ...
      const key = addFileCandidate(id, file, 'Added', true);
      selectCandidate(id, key, slotIndex);
    });
    wrap.append(input);
    return wrap;
  }
  ...
  function captureNativeSelection(event) {
    if (state.suppressNativeCapture) return;
  ...
  function addFileCandidate(id, file, source, select) {
    ...
    if (!cardState.candidates.some((candidate) => candidate.key === key)) {
      cardState.candidates.unshift({ key, source, label: file.name, previewUrl: URL.createObjectURL(file), file, revoke: true });
    }
  ...
  function fillNativeInput(slotIndex, file) {
    ...
    state.suppressNativeCapture = true;
    try {
      input.files = transfer.files;
      input.dispatchEvent(new Event('change', { bubbles: true }));
    } finally {
      state.suppressNativeCapture = false;
    }
  }
