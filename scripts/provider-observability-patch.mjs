...
export async function patchProviderObservability(distDirectory) {
  ...
  source = replaceRequired(source, '  function captureNativeSelection(event) {', [
    ...
    "      const rawFound = Number(status.rawFound ?? status.found) || 0;",
    "      const eligibleFound = Number(status.eligibleFound ?? status.found) || 0;",
    "      const loaded = Number(status.loaded) || 0;",
    "      const reviewed = Number(status.reviewed) || 0;",
    ...
    "        detailParts.push(`${labels[source]}: ${rawFound} raw, ${eligibleFound} eligible, ${loaded} loaded, ${reviewed} reviewed, ${count} accepted${errors ? `, ${errors} errors` : ''}`);",
    "      }",
    "    }",
    "    return {",
